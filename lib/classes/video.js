...
class Video {
    constructor(path, options) {
        ...
        this.output = {
            ...
            dir: this.options.temp,
            final: this.options.destination,
    ...
    encode() {
        ...
        return new Promise(function(resolve, reject) {
            ...
            if (_self.options.scale)
                _self.ffmpegCommand.videoFilters('scale=-4:\'min(' + _self.options.scale + ',ih)\'');
            ...
            // H265 tune
            if (_self.options.tune && _self.options.asPreset != 'none')
                _self.ffmpegCommand.outputOptions('-tune', _self.options.tune);
            
            // Reference Frames
            if (_self.options.ref && _self.options.ref != 3)
               _self._addX265Option('ref=' + _self.options.ref);
            
            // Weight B
            if (_self.options.weightb)
                _self._addX265Option('weightb=1');
            
            // B Frames
            if (_self.options.bframes && _self.options.bframes != 4)
               _self._addX265Option('bframes=' + _self.options.bframes);
            
            // H265 extra options
            if (_self.options.extraOptions)
                _self._addX265Option(_self.options.extraOptions);
            
            // Stats Frequency - Not availble in released ffmpeg yet 2021/02/16
            // _self.ffmpegCommand.outputOptions('-stats_period', _self.options.statsFrequency);
    ...
    move() {
        let _self = this;
        return new Promise(function(resolve, reject) {
            if (_self.options.replace) {
                const newDestination = Path.join(_self.final, _self.output.base);
		        ...
		        const newDestination = Path.join(_self.final, _self.output.base);
                _self.encoder.logger.debug('Moving file from temp to destination', newDestination);
                fs.move(_self.output.path, newDestination, function(err) {
                    if (err) return reject(err);
                    _self._updateDestination(_self.dir);
                    resolve();
                });
